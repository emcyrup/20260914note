...
### 3-7b. 公式 LINE をつなぐ（https にしたあと）

ゆあーずで LINE を使うと、次のことが LINE でできます：月予約利用希望の「入力のお願い」（入力ページの URL）、月間予定表を作ったあとの「ご利用日が決まりました」、空き枠・前日のお知らせ、保護者からの「予約」「空いてますか」への自動返信、スタッフのグループへの予約の増減のお知らせ。

**LINE 側（LINE Developers / LINE Official Account Manager）**
1. 公式アカウントを作る（LINE Official Account Manager）。設定 → Messaging API → 「Messaging API を利用する」でプロバイダーを選ぶ。
2. [LINE Developers](https://developers.line.biz/console/) でそのチャネルを開き、次の2つを控える（**チャットや Notion に貼らない**。画面にだけ入れる）。
   - チャネル基本設定 → **チャネルシークレット**
   - Messaging API 設定 → **チャネルアクセストークン（長期）** → 発行
3. Messaging API 設定 → **Webhook URL** に、アプリの「予約 → 公式LINE」の一番上に出る URL（`https://<ドメイン>/line/webhook/<施設ID>/`）を貼る → **Webhook の利用：オン** → 「検証」で成功を確認（アプリ側の2つを入れたあとに押す）。
4. LINE Official Account Manager → 応答設定：**応答メッセージ オフ**・**Webhook オン**（あいさつメッセージは任意）。

**アプリ側（管理者でログイン）**
5. 施設設定 → 施設基本情報 → **チャネルシークレット**と**チャネルアクセストークン**を入れて保存（保存後は画面に出ません）。施設設定 → 使う機能で「LINE連携」がオンか確認。
6. 3 の「検証」を押して成功すれば、つながっています。

**保護者の LINE を顧客台帳につなぐ**（お知らせを LINE で届けるため。どちらか）
- **登録コード（おすすめ）**：利用者台帳 → 保護者 → 「LINE登録コード」（8文字・72時間有効）を保護者に渡す → 保護者が公式アカウントを友だち追加してコードを送る。保護者台帳と**予約の顧客台帳の両方**につながります（同じ名前の顧客がいればその人に、いなければ作る。きょうだいのコードを続けて送れば、同じ LINE に足されます）。
- **受信箱から**：保護者が何か送ってくると「予約 → 公式LINE」の受信に出る → 「顧客台帳の人につなぐ」でその保護者を選ぶ。
- つながった保護者は、利用希望の「保護者に入力してもらう」で「LINE：つながっている」になり、「入力のお願いを送る」で LINE に届きます。

**スタッフのグループ（任意）**
7. 公式アカウントをスタッフの LINE グループに招待 → そのグループで何か1つ投稿 → 「予約 → 公式LINE」の「スタッフのグループからの投稿」で「この投稿のグループを通知先にする」。予約の増減・保護者から利用希望が届いたことがそのグループに届きます。
...
2. 「API とサービス」→「認証情報」→「認証情報を作成」→「API キー」。作ったキーの「キーを編集」で次の2つを制限する。
   - **API の制限**：「キーを制限」→ **Cloud Speech-to-Text API** だけ
   - **アプリケーションの制限**：「IP アドレス」→ ゆあーずのサーバーの固定 IP（`34.104.220.242`）。音声はサーバーから Google に送るので、ほかからは使えないようにする
3. キーは**チャットや Notion に貼らない**。次の手順でサーバーの `.env` にだけ入れる。

**サーバー側**
4. ブラウザで SSH → `sudo -u deploy nano /opt/ryoiku/.env` → 最後に `GOOGLE_SPEECH_API_KEY=<作ったキー>` の1行を足して保存。
5. `sudo -u deploy bash -lc "cd /opt/ryoiku && docker compose up -d"`（アプリを入れ直して設定を読み込ませる）。
6. iPhone で議事録の画面を開き直し、録音 → 止める → もう一度録音 を試す。話の区切りごとに文字が入れば使えています。うまくいかないときは「うまく文字にならないとき」→「記録を写す」の中身を見る（`server=true` なら設定は読み込まれています）。

**料金・データ**
- 送った音声の長さに応じた従量課金です（毎月の無料枠あり）。金額は Google の料金ページで確認してください。声の入っていない区切りは送りません。
- 音声は文字にするために Google に送るだけで、アプリのサーバーには残しません。
- キーを入れていない間は、iPhone ではこれまでどおりブラウザの音声認識を使います（2回目以降に文字にならないときは、画面にキーボードの音声入力を使う案内が出ます）。
