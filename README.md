...
**空き状況のページからのお申し込みは、その場では予約になりません。** アドレスを知っている方なら誰でも申し込めるため、届いた申し込みは「予約 → 申し込み」の画面に積まれ、職員がどの利用者のことかを確かめて「予約にする」を押したときだけ予約になります（知らない名前で枠が埋まらないようにするため）。見送り・顧客台帳への追加・記録の削除もこの画面で行います。設定「お子さまの名前が在籍者1人に決まるときは、その場で予約にする」を入れると、名前がちょうど1人に一致する場合だけ自動で予約になります（既定 off）。自動投稿よけの見えない欄と、同じ相手から1時間5件までの制限があります。
...
- **顧客（公式LINEへのメッセージ）**：顧客台帳にいる方が、日にちとお子さまの名前を書いて送ったときだけ反映し、その場で LINE に返事をします（`9/20 予約おねがいします` / `9/20 キャンセルします` / `空いてますか？`。1通に5日まで）。受け付ける範囲の外や、担当していない利用者の名前は反映せず、受信箱へ回します。既定は off（職員が確かめてから反映）
...
どちらも、読み取れなかった文はこれまでどおり受信箱に積み、職員が確かめてから反映します。
